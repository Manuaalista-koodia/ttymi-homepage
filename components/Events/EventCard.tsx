...
export const EventCard = ({ event }: { event: EventType }) => {
  ...
  return (
    ...
    >
      <div className='w-36 h-36 md:w-64 md:h-64 lg:w-full lg:h-64 flex-shrink-0'>{getImgComponent(event)}</div>
      <div className='relative flex-1 px-4 md:px-6 bg-white lg:-mt-12 lg:rounded-t-3xl min-w-0 flex flex-col justify-center lg:justify-start gap-1 md:gap-2'>
        <h2 className='text-xl md:text-3xl lg:mt-6 font-semibold leading-tight line-clamp-1'>{event.title}</h2>
        ...
        <p className='text-xs sm:text-sm md:text-lg font-medium leading-relaxed line-clamp-3 md:line-clamp-4 lg:line-clamp-6 opacity-80 mt-1 md:mt-2'>
          {event.content}
        </p>
      </div>
    </Link>
